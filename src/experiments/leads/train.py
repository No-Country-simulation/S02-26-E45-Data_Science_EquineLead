...
DAGSHUB_USER = "ITRoselloSignoris"  # Actualizado según tu perfil de GitHub
...
def train(data_path: str, outdir: str):
    ...
    # y_train eliminado para evitar F841 (variable no usada)
    y_test = datasets["y_test"]

    # Masks y targets auxiliares para evaluación (Formateados para evitar E501)
    ...
    y_test_p2_horse = (y_test["horse_target"][mask_test_p2_horse] == "Lead Oro").astype(
        int
    )

    y_test_p2_prods = (y_test["prods_target"][mask_test_p2_prods] == "Lead Oro").astype(
        int
    )
    ...
    check_overfitting(
        [
            (
                "P1 — horse",
                p1h,
                datasets["X_train_horse"],
                models["y_tr_p1h"],
                datasets["X_test_horse"],
                y_test_p1_horse,
            ),
            (
                "P1 — prods",
                p1p,
                datasets["X_train_prods"],
                models["y_tr_p1p"],
                datasets["X_test_prods"],
                y_test_p1_prods,
            ),
            (
                "P2 — horse",
                ...
                datasets["X_p2h_raw"],
                models["y_p2h_raw"],
                datasets["X_test_horse"][mask_test_p2_horse],
                y_test_p2_horse,
            ),
            (
                "P2 — prods",
                ...
                datasets["X_p2p_raw"],
                models["y_p2p_raw"],
                datasets["X_test_prods"][mask_test_p2_prods],
                y_test_p2_prods,
            ),
        ],
        RUN_NAME,
...
def tune(data_path: str):
    ...
    # Ajuste de líneas largas en targets de tuning
    y_p2h_raw = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", required=True, help="Ruta al df_final (.parquet o .csv)"
    )
    parser.add_argument(
        "--outdir",
        default="../../models/champion",
        help="Directorio de salida de modelos y artefactos",
    )
    parser.add_argument(
        "--tune", action="store_true", help="Ejecutar tuning antes de entrenar"
    )
