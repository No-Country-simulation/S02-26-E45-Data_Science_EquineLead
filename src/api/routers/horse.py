...
from schemas import HorsePredictRequest
...
def run_prediction(features: dict):
    ...
    # Loguear para monitoreo de drift
    logger.info(
        {
            "event": "horse_prediction",
            "prob_oro": result["paso2"]["prob_oro"],
            "prob_plata": result["paso2"]["prob_plata"],
            "prob_bronce": result["paso1"]["prob_bronce"],
            "horses_viewed": features.get("horses_viewed"),
            "ratio_cart_horse": features.get("ratio_cart_horse"),
        }
    )
...
@router.post("/predict")
def predict_horse(data: HorsePredictRequest):
    try:
