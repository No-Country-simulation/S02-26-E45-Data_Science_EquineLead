...
def clean_data(df_prod_clean: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in cols_texto:
        if col in df_prod_clean.columns:
            df_prod_clean[col] = (
                ...
                .str.strip()
                .replace(
                    {"nan": "unknown", "none": "unknown", "sin información": "unknown"}
                )
            )
    ...
    if "Price" in df_prod_clean.columns:
        df_prod_clean["Price"] = (
            df_prod_clean["Price"].astype(str).str.replace(r"[$,]", "", regex=True)
        )
        df_prod_clean["Price"] = pd.to_numeric(df_prod_clean["Price"], errors="coerce")
        df_prod_clean["Price"] = df_prod_clean["Price"].fillna(
            df_prod_clean["Price"].median()
        )

    if "Stock" in df_prod_clean.columns:
        df_prod_clean["Stock"] = (
            df_prod_clean["Stock"].astype(str).str.extract(r"(\d+)").astype(float)
        )
...
if __name__ == "__main__":
    ...
    df_prod_clean.to_parquet(
        PATH_OUTPUT / "products_listing_limpio.parquet", index=False
    )
