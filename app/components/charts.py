...
def plot_tam_distribution(df: pd.DataFrame):
    ...
    if col == "Temp_Country":
        df["Temp_Country"] = df["Location"].apply(
            lambda x: x.split(",")[-1].strip() if "," in str(x) else "Other"
        )
...
def plot_cpl_comparison():
    ...
    fig = px.bar(
        ...
        text="CPL (USD)",
        color="Etapa",
        color_discrete_map={"2023 (Estático)": "#ef4444", "2024 (MLE)": "#10b981"},
    )
    fig.update_traces(
...
def plot_traffic_seasonality(time_grain="Mensual"):
    ...
    fig.update_traces(
        line_color=COLOR_REAL,
        fill="tozeroy",
        fillcolor="rgba(17, 141, 255, 0.1)",
        line_width=4,
    )
    return apply_bi_layout(
        fig, f"Estacionalidad del Tráfico (K Sesiones) - {time_grain}"
    )
...
def plot_missing_values(df: pd.DataFrame):
    ...
    fig.add_hline(
        y=95, line_dash="dash", line_color="#10b981", annotation_text="SLA 95%"
    )
...
def plot_data_drift():
    ...
    fig = px.bar(
        x=weeks, y=drift_score, color=drift_score, color_continuous_scale="Reds"
    )
    fig.add_hline(
        y=0.1, line_dash="dash", line_color="#ef4444", annotation_text="THRESHOLD"
    )
...
def plot_feature_importance(algo="Random Forest"):
    ...
    fig.update_traces(
        textposition="outside", texttemplate="%{text:.2f}", cliponaxis=False
    )
...
def plot_roc_curve(algo="Random Forest"):
    ...
    fig.update_traces(
        line_color=COLOR_REAL,
        fill="tozeroy",
        fillcolor="rgba(17, 141, 255, 0.05)",
        line_width=4,
    )
    fig.add_shape(
        type="line", line=dict(dash="dash", color="#94a3b8"), x0=0, x1=1, y0=0, y1=1
    )
...
def plot_ab_test_results(df: pd.DataFrame):
    ...
    counts = (
        df.groupby(["experiment_group", "event_type"]).size().reset_index(name="count")
    )
...
def plot_confidence_intervals():
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(size=14, color=["#ef4444", "#10b981"]),
        )
    )
...
def plot_average_marginal_effects():
    ...
    effects = {
        "Premium Pedigree": -0.02,
        "Con Video": 0.05,
        "Hook Emocional": 0.08,
        "Precio Alto": -0.04,
    }
    fig = px.bar(
        x=list(effects.values()),
        y=list(effects.keys()),
        orientation="h",
...
def plot_funnel():
    ...
    data = dict(
        number=[50000, 15000, 6000, 800],
        stage=["Impresiones", "Clicks", "Vistas Detalle", "Leads B2B"],
    )
...
def plot_roi_projection(cost_monthly: float, incremental_revenue: list, months: list):
    ...
    fig.add_trace(
        go.Bar(
            ...
            name="Revenue Extra",
            marker=dict(
                color="#10b981", line=dict(color="rgba(255,255,255,0.1)", width=1)
            ),
        )
    )
    fig.add_trace(
...
def plot_break_even(costo_lead=25, costo_squad=15000):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            fill="tonexty",
            fillcolor="rgba(18, 179, 171, 0.05)",
        )
    )
    fig.add_trace(
        go.Scatter(
            x=leads_sold, y=cost, name="Costs", line=dict(color="#ef4444", width=3)
        )
    )
