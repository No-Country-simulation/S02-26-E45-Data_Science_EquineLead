...
def render_conversion_analytics(df_u_sessions, df_p_sessions):
    ...
    with r1_c1:
        if not filtered_u.empty and 'event_type' in filtered_u.columns:
            ...
            st.plotly_chart(apply_pro_chart_style(fig_he, "Horse Interaction Funnel"), use_container_width=True)
    with r1_c2:
        if not filtered_p.empty and 'event_type' in filtered_p.columns:
            ...
            st.plotly_chart(apply_pro_chart_style(fig_pe, "Retail Funnel Progression"), use_container_width=True)
    ...
    with r2_c1:
        if not filtered_u.empty and 'event_time' in filtered_u.columns:
            ...
            st.plotly_chart(apply_pro_chart_style(fig_ht, "Horse Event Volume Over Time"), use_container_width=True)
    with r2_c2:
        if not filtered_p.empty and 'event_time' in filtered_p.columns:
            ...
            st.plotly_chart(apply_pro_chart_style(fig_pt, "Retail Event Volume Over Time"), use_container_width=True)
    ...
    with r3_c1:
        if not filtered_u.empty and 'horse_id' in filtered_u.columns:
            ...
            st.plotly_chart(apply_pro_chart_style(fig_th, "Top 10 High-Velocity Horses"), use_container_width=True)
    with r3_c2:
        if not filtered_p.empty and 'item_id' in filtered_p.columns:
            ...
            st.plotly_chart(apply_pro_chart_style(fig_tp, "Top 10 High-Velocity Products"), use_container_width=True)
