...
def render_executive_summary(
    ...
    st.markdown(
        "Real-time aggregate performance indicators across the EquineLead ecosystem."
    )
    ...
    kpi1.metric(
        "Active Global Users",
        f"{len(df_users):,}" if not df_users.empty else "0",
        "12% MoM",
    )
    kpi2.metric(
        "Horses in Marketplace",
        f"{len(df_horses):,}" if not df_horses.empty else "0",
        "4.2% MoM",
    )
    kpi3.metric(
        "Retail Products",
        f"{len(df_products):,}" if not df_products.empty else "0",
        "8.9% MoM",
    )
    ...
    with r2_c1:
        if not df_users.empty and "first_seen" in df_users.columns:
            Growth = (
                df_users.groupby(df_users["first_seen"].dt.date)
                .size()
            ...
            fig_acq.add_scatter(
                ...
                mode="lines",
                name="Daily New",
                line=dict(color=PRO_COLORS[1], width=2, dash="dot"),
            )
            st.plotly_chart(
                apply_pro_chart_style(
                    fig_acq, "Cumulative Network Growth vs Daily Acquisition"
                ),
    ...
    with r2_c2:
        if not df_users.empty and "traffic_source" in df_users.columns:
            ...
            st.plotly_chart(
                apply_pro_chart_style(fig_traffic, "Traffic Source Origin"),
                use_container_width=True,
            )
    ...
    with r3_c1:
        if not df_users.empty and "country" in df_users.columns:
            ...
            st.plotly_chart(
                apply_pro_chart_style(fig_geo, "Global Penetration Map"),
                use_container_width=True,
            )

    with r3_c2:
        if not df_u_sessions.empty:
            df_u_sessions["date"] = pd.to_datetime(
                df_u_sessions["event_time"], errors="coerce"
            ).dt.date
            h_ev = df_u_sessions.groupby("date").size().reset_index(name="Horse")

            if not df_p_sessions.empty:
                df_p_sessions["date"] = pd.to_datetime(
                    df_p_sessions["event_time"], errors="coerce"
                ).dt.date
            ...
            merged_ev = (
                pd.merge(h_ev, p_ev, on="date", how="outer")
                .fillna(0)
                .sort_values("date")
            )
            ...
            fig_stack.add_trace(
                go.Bar(
                    ...
                    y=merged_ev["Product"],
                    name="Product Events",
                    marker_color=PRO_COLORS[3],
                )
            )
            fig_stack.update_layout(barmode="stack")
            st.plotly_chart(
                apply_pro_chart_style(
                    fig_stack, "Aggregated Platform Usage (Interactions)"
                ),
                use_container_width=True,
            )
